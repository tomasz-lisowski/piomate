...
const microzig__pioasm = @import("/home/w/Desktop/piomate/src/microzig/port/raspberrypi/rp2xxx/src/hal/pio/assembler.zig");
const microzig__pioasm__encoder = @import("/home/w/Desktop/piomate/src/microzig/port/raspberrypi/rp2xxx/src/hal/pio/assembler/encoder.zig");
const microzig__pioasm__tokenizer = @import("/home/w/Desktop/piomate/src/microzig/port/raspberrypi/rp2xxx/src/hal/pio/assembler/tokenizer.zig");
...
fn encodeDelaySideset(buffer: []u8, ins_encode__len: usize, delay_sideset: u5, sideset__config: ConfigSideset) ![]const u8 {
    ...
    const delay__encode: ?[]const u8 = if (delay__raw != 0) try std.fmt.bufPrint(buffer[buffer__start + side_encode__len ..], "{s}[{d}]", .{ if (side_encode__len > 0) " " else "          ", delay__raw.? }) else null;
...
fn encodeJmp(instruction: microzig__pioasm__encoder.Instruction, buffer: []u8, instruction__index_last: u5) ![]const u8 {
    ...
    return try std.fmt.bufPrint(buffer, "JMP     {s}{s}__0x{X:0>2}", .{ condition__text, if (instruction.payload.jmp.address > instruction__index_last) "OUTSIDE_PROGRAM" else label__name[instruction.payload.jmp.address], instruction.payload.jmp.address });
}
...
fn encodeWait(instruction: microzig__pioasm__encoder.Instruction, buffer: []u8) ![]const u8 {
    ...
    return try std.fmt.bufPrint(buffer, "WAIT    {d} {s}, {d}", .{ instruction.payload.wait.polarity, source__text, instruction.payload.wait.index });
}
...
fn encodeIn(instruction: microzig__pioasm__encoder.Instruction, buffer: []u8) ![]const u8 {
    ...
    return try std.fmt.bufPrint(buffer, "IN      {s}, {d}", .{ source_text, if (instruction.payload.in.bit_count == 0) 32 else @as(u6, instruction.payload.in.bit_count) });
}
...
fn encodeOut(instruction: microzig__pioasm__encoder.Instruction, buffer: []u8) ![]const u8 {
    ...
    return try std.fmt.bufPrint(buffer, "OUT     {s}, {d}", .{ destination_text, if (instruction.payload.out.bit_count == 0) 32 else @as(u6, instruction.payload.out.bit_count) });
}
...
fn encodeIrq(instruction: microzig__pioasm__encoder.Instruction, buffer: []u8) ![]const u8 {
    ...
    return try std.fmt.bufPrint(buffer, "IRQ     {s} {d}{s}", .{ operation__text, irq__idx, if (irq__rel == 1) " rel" else "" });
}
